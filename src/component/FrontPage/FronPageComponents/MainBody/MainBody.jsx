...
const Main = () => {
  return (
    <div className={style["Main"]}>
      <div className="row m-0 justify-content-around flex-wrap">
        {product.map((e) => {
          return (
            <div
              key={e.title}
              className={style["cardCont"] + " card col-3 p-0 border-0"}
            >
              <img src={e.image} className="card-img-top" alt="..." />
              <div className="card-body">
                <h5
                  className="card-title"
                  style={{ fontSize: "15.9px", fontWeight: "bold" }}
                >
                  {e.title}
                </h5>
                ...
                <Link to="/pages/ipad" className="btn btn-info">
                  {e.button_des}
                </Link>
              </div>
            </div>
